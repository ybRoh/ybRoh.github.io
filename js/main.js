...
(function () {
    ...
    function loadProjects() {
        ...
        var apiUrl = 'https://api.github.com/users/ybRoh/repos?sort=updated&per_page=100';
        ...
        fetch(apiUrl)
            ...
            .then(function (repos) {
                // fork 제외, .github.io 사이트 자체 제외
                var filtered = repos.filter(function (repo) {
                    return !repo.fork && repo.name !== 'ybRoh.github.io';
                });
                ...
                if (footerList) {
                    var footerHtml = '';
                    filtered.forEach(function (repo) {
                        footerHtml += '<li><a href="' + escapeHtml(repo.html_url) + '" target="_blank" rel="noopener">' + escapeHtml(repo.name) + '</a></li>';
                    });
                ...
                filtered.forEach(function (repo) {
                    ...
                    var desc = repo.description || '설명이 없습니다.';
                    ...
                    html += '    <span class="meta-item">' + formatDate(repo.updated_at) + ' 업데이트</span>';
                    html += '  </div>';
                    ...
                    if (repo.topics) {
                        repo.topics.forEach(function (topic) {
                            html += '    <span class="tech-badge">' + escapeHtml(topic) + '</span>';
                        });
                    }
                    html += '  </div>';
                    html += '  <div class="product-actions">';
                    html += '    <a href="' + escapeHtml(repo.html_url) + '/releases" class="btn btn-primary btn-sm" target="_blank" rel="noopener">';
                    ...
                    html += '      다운로드';
                    html += '    </a>';
                    html += '    <a href="' + escapeHtml(repo.html_url) + '" class="btn btn-ghost btn-sm" target="_blank" rel="noopener">';
